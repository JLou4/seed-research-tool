...
export async function searchStartups(keywords, limit = 20) {
  ...
  // TIER 2: Database & publication searches + TOP SEED VCs/ACCELERATORS
  const tier2Queries = [
    // TechCrunch stealth launches
    `site:techcrunch.com "${keywords}" "stealth" OR "seed round" OR "raises" 2024 2025`,
    
    // arXiv papers → startups (author affiliations)
    `site:arxiv.org ${keywords} startup OR company OR founded`,
    
    // === TOP SEED VCs & ACCELERATORS ===
    // Y Combinator (already in tier 1)
    
    // Techstars
    `site:techstars.com/portfolio ${keywords}`,
    `"Techstars" "${keywords}" portfolio 2024 2025`,
    
    // 500 Global (formerly 500 Startups)
    `site:500.co/startups ${keywords}`,
    `"500 Global" OR "500 Startups" "${keywords}" portfolio`,
    
    // SV Angel
    `site:svangel.com ${keywords} portfolio`,
    `"SV Angel" "${keywords}" investment seed`,
    
    // BoxGroup
    `site:boxgroup.com ${keywords}`,
    `"BoxGroup" "${keywords}" portfolio seed`,
    
    // Founder Collective
    `site:foundercollective.com/portfolio ${keywords}`,
    `"Founder Collective" "${keywords}" seed`,
    
    // Haystack
    `site:haystack.vc ${keywords}`,
    `"Haystack VC" OR "Haystack Ventures" "${keywords}" seed`,
    
    // Neo
    `site:neo.com/portfolio ${keywords}`,
    `"Neo VC" "${keywords}" seed early stage`,
    
    // Boost VC
    `site:boost.vc/portfolio ${keywords}`,
    `"Boost VC" "${keywords}" pre-seed seed`,
    
    // Precursor Ventures
    `site:precursorvc.com ${keywords}`,
    `"Precursor Ventures" "${keywords}" pre-seed`,
    
    // First Round Capital
    `site:firstround.com/portfolio ${keywords}`,
    `"First Round Capital" "${keywords}" seed`,
    
    // Floodgate
    `site:floodgate.com/portfolio ${keywords}`,
    `"Floodgate" "${keywords}" seed thunder`,
    
    // === OTHER TOP SEED SOURCES ===
    `site:a16z.com OR site:sequoia.com OR site:greylock.com ${keywords} seed`,
    
    // General startup funding (SEED/PRE-SEED/SERIES A ONLY)
    `${keywords} startup "seed round" 2024 2025`,
    `${keywords} startup "pre-seed" 2024 2025`,
    `${keywords} "Series A" early stage 2024 2025`,
    `${keywords} startup founded 2023 2024 2025 seed`,
  ];
