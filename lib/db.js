...
export async function initDb() {
  ...
  // Run schema - split into individual statements (neon doesn't have .unsafe())
  const statements = [
  ...
  // NEW: Add funding_stage, last_funding_type, and sources columns
  try {
    await sql`ALTER TABLE companies ADD COLUMN IF NOT EXISTS funding_stage VARCHAR(50)`;
    console.log('Migration: funding_stage column OK');
  } catch (e) {
    console.error('Migration ERROR (funding_stage):', e.message);
    errors.push(`funding_stage: ${e.message}`);
  }
  
  try {
    await sql`ALTER TABLE companies ADD COLUMN IF NOT EXISTS last_funding_type VARCHAR(50)`;
    console.log('Migration: last_funding_type column OK');
  } catch (e) {
    console.error('Migration ERROR (last_funding_type):', e.message);
    errors.push(`last_funding_type: ${e.message}`);
  }
  
  try {
    await sql`ALTER TABLE companies ADD COLUMN IF NOT EXISTS sources JSONB`;
    console.log('Migration: sources column OK');
  } catch (e) {
    console.error('Migration ERROR (sources):', e.message);
    errors.push(`sources: ${e.message}`);
  }
  
  // NEW: thesis_sources column for validating sources (patents, research, articles)
  try {
    await sql`ALTER TABLE theses ADD COLUMN IF NOT EXISTS thesis_sources JSONB`;
    console.log('Migration: thesis_sources column OK');
  } catch (e) {
    console.error('Migration ERROR (thesis_sources):', e.message);
    errors.push(`thesis_sources: ${e.message}`);
  }
  
  // NEW: url_valid column for background URL validation
  try {
    await sql`ALTER TABLE companies ADD COLUMN IF NOT EXISTS url_valid BOOLEAN`;
    console.log('Migration: url_valid column OK');
  } catch (e) {
    console.error('Migration ERROR (url_valid):', e.message);
    errors.push(`url_valid: ${e.message}`);
  }
