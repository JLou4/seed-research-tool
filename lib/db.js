...
export async function initDb() {
  ...
  // Add adjacent_themes column if it doesn't exist (migration for existing tables)
  await sql`
    ALTER TABLE theses ADD COLUMN IF NOT EXISTS adjacent_themes JSONB
  `.catch(() => {});
  
  // Add discovery_stats column
  await sql`
    ALTER TABLE theses ADD COLUMN IF NOT EXISTS discovery_stats JSONB
  `.catch(() => {});
  ...
  // Add fit_type and discovered_via_theme columns if they don't exist (migration)
  await sql`ALTER TABLE companies ADD COLUMN IF NOT EXISTS fit_type VARCHAR(20)`.catch(() => {});
  await sql`ALTER TABLE companies ADD COLUMN IF NOT EXISTS discovered_via_theme VARCHAR(255)`.catch(() => {});
