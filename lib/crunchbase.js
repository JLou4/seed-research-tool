...
export async function searchOrganizations(query, limit = 10) {
  ...
  try {
    // Try the search endpoint with simpler query
      ...
      body: JSON.stringify({
        ...
        query: [
          ...
          },
          {
            ...
            values: [query.split(' ')[0]]  // Single keyword for broader results
          }
        ],
        order: [
    ...
    if (!response.ok) {
      ...
      // If search fails, the API key might not have search access
      // Return empty but log the issue clearly
      return [];
    }

    const data = await response.json();
    console.log(`[Crunchbase] Found ${data.entities?.length || 0} results`);
