...
export async function* runResearchSimple(thesis) {
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: 6000,
      // No extended thinking - just a good prompt (Vercel 60s limit)
      messages: [{
        role: 'user',
        content: `${RESEARCH_PROMPT}\n\nINVESTMENT THESIS: "${thesis}"\n\nIMPORTANT: Only include companies you are 100% certain exist. If you're not sure a company is real, leave it out. 7-10 high-confidence companies is better than 10 with guesses.`
      }],
    });
