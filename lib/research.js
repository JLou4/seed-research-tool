...
import { enrichCompany, isCrunchbaseAvailable } from './crunchbase.js';
...
Find 5-8 relevant early-stage companies. Focus on pre-seed, seed, and Series A companies. Prioritize companies that are less obvious but highly aligned with the thesis.`;
  ...
  try {
    ...
    // Check if Crunchbase enrichment is available
    const useCrunchbase = isCrunchbaseAvailable();
    if (useCrunchbase) {
      yield { type: 'progress', message: 'Enriching with Crunchbase data...' };
    }
    
    // Yield each company, enriched with Crunchbase data if available
    for (const company of result.companies || []) {
      let enrichedCompany = company;
      
      if (useCrunchbase) {
        yield { type: 'progress', message: `Verifying ${company.name} on Crunchbase...` };
        enrichedCompany = await enrichCompany(company);
      }
      
      yield { type: 'company', data: enrichedCompany };
    }
