...
/**
 * STEP 2.5b: VERIFICATION FILTER - Only keep companies with trusted data
 * For investment decisions, accuracy > coverage
 */
function filterToVerifiedOnly(companies, progressCallback) {
  const verified = companies.filter(c => {
    // Must have Crunchbase verification
    if (!c.crunchbase_verified) {
      console.log(`Excluding unverified company: ${c.name}`);
      return false;
    }
    // Must have a real description (not empty/null)
    if (!c.description || c.description.length < 10) {
      console.log(`Excluding company with no description: ${c.name}`);
      return false;
    }
    return true;
  });
  
  progressCallback(`${verified.length} companies have verified Crunchbase data (filtered from ${companies.length})`);
  return verified;
}
...
STRICT RULES (violation = unusable output):
1. ONLY analyze the companies listed below - never invent or add companies
2. Use ONLY the description provided for each company - this is verified Crunchbase data
3. Do NOT guess what a company does based on its name - "Cambio" is NOT a "change platform"
4. If the provided description doesn't fit the thesis, score it LOW - don't force a fit
5. If you're uncertain about a company, say so explicitly

For each company, write a brief investment memo:
1. A 2-3 paragraph analysis BASED ONLY ON THE PROVIDED DESCRIPTION:
   - How does this company (as described) fit the thesis?
   - What's the potential opportunity?
   - Key risks or concerns?
2. Scores (1-10, be honest):
   - thesis_relevance: How well does the ACTUAL description match the thesis?
   - recency: Based on founding date if provided (10 = 2023+, 5 = 2020-2022, 1 = older/unknown)
   - founding_team: 5 = unknown (default), only adjust if you have VERIFIED info
...
async function analyzeCompanies(thesis, companies, searchTerms, progressCallback) {
  ...
  // Prepare company list with VERIFIED data only
  const companyList = topCompanies.map(c => {
    let info = `- ${c.name} [VERIFIED via Crunchbase]`;
    info += `\n  Description: "${c.description}"`;  // Quote to emphasize this is exact data
    if (c.website) info += `\n  Website: ${c.website}`;
...
===== VERIFIED COMPANIES (Crunchbase data - use ONLY these descriptions) =====
${companyList}
===== END COMPANY LIST =====

Analyze each company based ONLY on the verified description above. Do not make assumptions beyond what's stated.`;
...
export async function* runResearch(thesis) {
  ...
  try {
    ...
    // Step 2.5b: VERIFICATION FILTER - Only keep companies with trusted data
    // For investment decisions, accuracy > coverage
    const verifiedCompanies = filterToVerifiedOnly(realCompanies, (msg) => {});
    yield { type: 'progress', message: `${verifiedCompanies.length} companies verified via Crunchbase (accuracy filter)` };
    
    if (verifiedCompanies.length === 0) {
      yield { type: 'progress', message: 'No verified companies found. Crunchbase data required for accuracy.' };
      yield {
        type: 'complete',
        data: {
          companies: [],
          public_comps: searchTerms.public_comps || [],
          summary: 'No companies with verified Crunchbase data found for this thesis. Try a more specific thesis.',
        }
      };
      return;
    }
    ...
    // ONLY using verified companies now
    const filteredCompanies = await quickFitFilter(thesis, verifiedCompanies, searchTerms, (msg) => {});
    ...
    if (filteredCompanies.length === 0) {
      ...
      yield {
        ...
        data: {
          companies: [],
          public_comps: searchTerms.public_comps || [],
          summary: `Found ${verifiedCompanies.length} verified companies but none scored 7+ for fit with: ${thesis}`,
        }
      };
      return;
    }
