...
/**
 * STEP 6: Generate adjacent themes based on thesis + discovered companies
 * These are related investment areas the user might want to explore next
 */
const ADJACENT_THEMES_PROMPT = `You are a seed-stage investment analyst helping expand an investor's deal flow.

Based on the investment thesis and the companies discovered, suggest adjacent/related investment themes.

Think about:
- Enabling technologies (what needs to exist for these companies to succeed?)
- Supply chain plays (who supplies/distributes to these companies?)
- Second-order effects (if this thesis succeeds, what else grows?)
- Parallel markets (same tech, different industry application)
- Complementary sectors (what goes hand-in-hand with this?)

Return JSON only:
{
  "adjacent_themes": [
    {
      "theme": "Theme name (4-6 words)",
      "description": "One sentence explaining why this is related/interesting",
      "example_companies": ["Known company 1", "Known company 2"],
      "connection": "How this relates to the original thesis"
    }
  ]
}

Generate 5-8 high-quality, specific themes. Avoid generic suggestions like "AI" or "SaaS" - be specific.`;

async function generateAdjacentThemes(thesis, companies, searchTerms) {
  try {
    // Prepare context from discovered companies
    const companyContext = companies.slice(0, 5).map(c => 
    ...
    const response = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `${ADJACENT_THEMES_PROMPT}
...
INITIAL ADJACENT THEMES (from thesis analysis):
${(searchTerms.adjacent_themes || []).join(', ')}

Generate NEW adjacent themes that expand on these findings - focus on actionable investment areas.`
      }],
    });
    ...
    const jsonMatch = text.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
      // Fallback to initial themes
      return (searchTerms.adjacent_themes || []).map(t => ({
        theme: t,
        description: '',
        example_companies: [],
        connection: 'Related to original thesis'
      }));
    }
    
    const result = JSON.parse(jsonMatch[0]);
    return result.adjacent_themes || [];
  } catch (error) {
    console.error('Adjacent themes generation error:', error);
    // Fallback to initial themes from search terms
    return (searchTerms.adjacent_themes || []).map(t => ({
      theme: t,
      description: '',
      example_companies: [],
...
export async function* runResearch(thesis) {
  ...
  try {
    ...
    // Step 6: Generate expanded adjacent themes based on actual findings
    yield { type: 'progress', message: 'Generating adjacent investment themes...' };
    const expandedThemes = await generateAdjacentThemes(thesis, enrichedCompanies, searchTerms);
    ...
    yield {
      ...
      data: {
        ...
        adjacent_themes: expandedThemes,
      }
    };
